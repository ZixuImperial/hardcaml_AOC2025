...
let generate_day4_rtl () =
  let module C = Circuit.With_interface (Day4.I) (Day4.O) in
  let scope = Scope.create ~auto_label_hierarchical_ports:true () in
  let circuit = C.create_exn ~name:"day1_top" (Day4.hierarchical scope) in
  let rtl_circuits =
    Rtl.create ~database:(Scope.circuit_database scope) Verilog [ circuit ]
  in
  let rtl = Rtl.full_hierarchy rtl_circuits |> Rope.to_string in
...
let () =
  Command_unix.run
    (Command.group ~summary:"" [ ("range-finder", range_finder_rtl_command) ; ("day1", day1_rtl_command); ("day4", day4_rtl_command) ; ("day7", day7_rtl_command)])
;;
